...
for checkout_ref in "${db_backward_only_refs[@]}" "${db_forward_no_options_refs[@]}" "${db_forward_with_options_refs[@]}" "${ext_backward_only_refs[@]}" "${ext_forward_refs[@]}" "${bak_backward_only_refs[@]}" "${bak_forward_refs[@]}"; do
...
for checkout_ref in "${checkout_refs[@]}"; do
	...
	if member_of_array "$checkout_ref" "${ext_backward_only_refs[@]}" \
		|| member_of_array "$checkout_ref" "${ext_forward_refs[@]}"; then
	...
	if member_of_array "$checkout_ref" "${ext_forward_refs[@]}"; then
	...
	if member_of_array "$checkout_ref" "${db_forward_no_options_refs[@]}" \
		|| member_of_array "$checkout_ref" "${db_forward_with_options_refs[@]}"; then
	...
	if member_of_array "$checkout_ref" "${db_forward_with_options_refs[@]}"; then
	...
	if member_of_array "$checkout_ref" "${bak_backward_only_refs[@]}" \
		|| member_of_array "$checkout_ref" "${bak_forward_refs[@]}"; then
		echo "== Use $checkout_ref to backup DB"
		backup_db $db_test_dir/$checkout_ref $bak_test_dir/$checkout_ref
	fi

	if member_of_array "$checkout_ref" "${bak_forward_refs[@]}"; then
